...
node-sass supports different configuration parameters to change settings related to the sass binary such as binary owner, binary path or alternative download path. Following parameters are supported by node-sass:

Variable owner    | .npmrc parameter | Process argument   | Value
-----------------|------------------|--------------------|------
SASS_BINARY_NAME | sass_binary_name | --sass-binary-owner | path
