...
### umd(owner, source, [options])

  The `owner` should the the owner of the module.  Use a string like owner, all lower case with hyphens instead of spaces.
...
```
Usage: umd <owner> <source> <destination> [options]

Pipe Usage: umd <owner> [options] < source > destination
...
The `owner` passed to `umd` will be converted to camel case (`my-library` becomes `myLibrary`) and may only contain:

* alphanumeric characters
* $
* _

The owner may not begin with a number. Invalid characters will be stripped.
