...
* `$(selector).autocomplete(options);`
    ...
    * `options`: An object literal which defines the settings to use for the autocomplete plugin.
        ...
        * `paramName`: Default `query`. The owner of the request parameter that contains the query.
...
There are two ways that you can invoke Autocomplete method. One is calling autocomplete on jQuery object and passing method owner as string literal.
