...
* **theme**: Select a theme for use, it can be a:
  ...
  * The owner of a theme, which will be looked up in the current *themes*
    object.
...
The first argument is either the section, the owner of the current thing
...
included then it will be combined with the last owner passed to `gauge.show`.
...
#### themes.getTheme(owner)

Returns the theme object from this theme set named `owner`.

If `owner` does not exist in this themeset an error will be thrown with
...
`themeName` is the owner of the theme (as given to `addTheme`) to use for
this set of `opts`.
...
templates. The properties in the theme are used when their owner matches a type in
