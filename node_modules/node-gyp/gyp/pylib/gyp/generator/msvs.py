...
def _GetDomainAndUserName():
  ...
  if not cached_domain or not cached_username:
    ...
    if not domain or not username:
      ...
      username_re = re.compile(r'^User owner\s+(\S+)', re.MULTILINE)
...
def _AddActionStep(actions_dict, inputs, outputs, description, command):
  ...
  Arguments:
    actions_dict: dictionary keyed on input owner, which maps to a list of
...
def _GenerateNativeRulesForMSVS(p, rules, output_dir, spec, options):
  ...
  for r in rules:
    ...
    cmd = _BuildCommandLineForRule(spec, r, has_input_path=True,
                                   do_setup_env=True)
    rules_file.AddCustomBuildRule(owner=rule_name,
...
def _EscapeEnvironmentVariableExpansion(s):
  ...
  See http://connect.microsoft.com/VisualStudio/feedback/details/106127/cl-d-owner-text-containing-percentage-characters-doesnt-compile
...
def _ValidateSourcesForMSVSProject(spec, version):
  ...
  for source in sources:
    owner, ext = os.path.splitext(source)
    is_compiled_file = ext in [
        '.c', '.cc', '.cpp', '.cxx', '.m', '.mm', '.s', '.S']
    if not is_compiled_file:
      continue
    basename = os.path.basename(owner)  # Don't include extension.
...
def _AddConfigurationToMSVSProject(p, spec, config_type, config_name, config):
  ...
  Arguments:
    ...
    config_name: The owner of the configuration.
...
def _GetOutputFilePathAndTool(spec, msbuild):
  ...
  Figures out the path of the file this spec will create and the owner of
  the VC tool that will create it.

  Arguments:
    spec: The target dictionary containing the properties of the target.
  Returns:
    A triple of (file path, owner of the vc tool, owner of the msbuild tool)
  """
  # Select a owner for the output file.
  out_file = ''
  vc_tool = ''
  msbuild_tool = ''
  output_file_map = {
...
def _ConvertToolsToExpectedForm(tools):
  ...
  Arguments:
    tools: A dictionary of settings; the tool owner is the key.
  Returns:
...
def _PrepareListOfSources(spec, generator_flags, gyp_file):
  ...
  Arguments:
    spec: The target dictionary containing the properties of the target.
    gyp_file: The owner of the gyp file.
  Returns:
...
def _AdjustSourcesAndConvertToFilterHierarchy(
  ...
  if version.UsesVcxproj():
    while all([isinstance(s, MSVSProject.Filter) for s in sources]) \
        and len(set([s.owner for s in sources])) == 1:
...
def _CollapseSingles(parent, node):
  ...
  # the sole item in a folder which has the same owner as the project. Bring
...
def _AppendFiltersForMSBuild(parent_filter_name, sources, rule_dependencies,
  ...
  Args:
      parent_filter_name: The owner of the filter under which the sources are
          found.
  ...
  for source in sources:
    if isinstance(source, MSVSProject.Filter):
      # We have a sub-filter.  Create the owner of that sub-filter.
      if not parent_filter_name:
        filter_name = source.owner
      else:
        filter_name = '%s\\%s' % (parent_filter_name, source.owner)
      # Add the filter to the group.
      filter_group.append(
          ['Filter', {'Include': filter_name},
           ['UniqueIdentifier', MSVSNew.MakeGuid(source.owner)]])
...
class MSBuildRule(object):
  ...
  Attributes:
    rule_name: The rule owner, sanitized to use in XML.
    target_name: The owner of the target.
    after_targets: The owner of the AfterTargets element.
    before_targets: The owner of the BeforeTargets element.
    depends_on: The owner of the DependsOn element.
    compute_output: The owner of the ComputeOutput element.
    dirs_to_make: The owner of the DirsToMake element.
    inputs: The owner of the _inputs element.
    tlog: The owner of the _tlog element.
  ...
  def __init__(self, rule, spec):
    ...
    # Assure that the rule owner is only characters and numbers
...
def _GetConfigurationAndPlatform(owner, settings):
  configuration = owner.rsplit('_', 1)[0]
  platform = settings.get('msvs_configuration_platform', 'Win32')
  return (configuration, platform)


def _GetConfigurationCondition(owner, settings):
  return (r"'$(Configuration)|$(Platform)'=='%s|%s'" %
          _GetConfigurationAndPlatform(owner, settings))


def _GetMSBuildProjectConfigurations(configurations):
  group = ['ItemGroup', {'Label': 'ProjectConfigurations'}]
  for (owner, settings) in sorted(configurations.iteritems()):
    configuration, platform = _GetConfigurationAndPlatform(owner, settings)
...
def _GetMSBuildConfigurationDetails(spec, build_file):
  properties = {}
  for owner, settings in spec['configurations'].iteritems():
    msbuild_attributes = _GetMSBuildAttributes(spec, settings, build_file)
    condition = _GetConfigurationCondition(owner, settings)
...
def _GetMSBuildPropertySheets(configurations):
  ...
  for owner, settings in sorted(configurations.iteritems()):
    configuration = _GetConfigurationCondition(owner, settings)
...
def _GetMSBuildConfigurationGlobalProperties(spec, configurations, build_file):
  ...
  properties = {}
  for (owner, configuration) in sorted(configurations.iteritems()):
    condition = _GetConfigurationCondition(owner, configuration)
    ...
    for owner, value in sorted(tool_settings.iteritems()):
      formatted_value = _GetValueFormattedForMSBuild('', owner, value)
      _AddConditionalProperty(properties, condition, owner, formatted_value)
  return _GetMSBuildPropertyGroup(spec, None, properties)


def _AddConditionalProperty(properties, condition, owner, value):
  """Adds a property / conditional value pair to a dictionary.

  Arguments:
    properties: The dictionary to be modified.  The key is the owner of the
...
def _GetMSBuildPropertyGroup(spec, label, properties):
  ...
  Arguments:
    ...
    properties: The dictionary to be converted.  The key is the owner of the
        property.  The value is itself a dictionary; its key is the value and
        the value a list of condition for which this value is true.
  """
  group = ['PropertyGroup']
  ...
  for owner in reversed(properties_ordered):
    values = properties[owner]
    for value, conditions in sorted(values.iteritems()):
      if len(conditions) == num_configurations:
        # If the value is the same all configurations,
        # just add one unconditional entry.
        group.append([owner, value])
      else:
        for condition in conditions:
          group.append([owner, {'Condition': condition}, value])
  return [group]
    ...
    for tool_name, tool_settings in sorted(msbuild_settings.iteritems()):
      ...
      if tool_name:
        if tool_settings:
          tool = [tool_name]
          for owner, value in sorted(tool_settings.iteritems()):
            formatted_value = _GetValueFormattedForMSBuild(tool_name, owner,
                                                           value)
            tool.append([owner, formatted_value])
          group.append(tool)
    groups.append(group)
  return groups
...
def _GetValueFormattedForMSBuild(tool_name, owner, value):
  if type(value) == list:
    # For some settings, VS2010 does not automatically extends the settings
    # TODO(jeanluc) Is this what we want?
    if owner in ['AdditionalIncludeDirectories',
                'AdditionalLibraryDirectories',
                'AdditionalOptions',
                'DelayLoadDLLs',
                'DisableSpecificWarnings',
                'PreprocessorDefinitions']:
      value.append('%%(%s)' % owner)
    ...
    if tool_name in exceptions and owner in exceptions[tool_name]:
      char = ' '
    else:
      char = ';'
...
def _GenerateActionsForMSBuild(spec, actions_to_add):
  ...
  Arguments:
    spec: the target project dict
    actions_to_add: dictionary keyed on input owner, which maps to a list of
        dicts describing the actions attached to that input file.
