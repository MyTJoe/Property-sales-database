...
class Target(object):
  ...
  owner: fully qualified owner of the target.
...
def _GenerateTargets(data, target_list, target_dicts, toplevel_dir, files,
  ...
  . A dictionary mapping from fully qualified owner to Target.
  ...
  # Maps from target owner to Target.
  name_to_target = {}
...
def _GetUnqualifiedToTargetMapping(all_targets, to_find):
  """Returns a tuple of the following:
  . mapping (dictionary) from unqualified owner to Target for all the
    Targets in |to_find|.
...
def _DoesTargetDependOnMatchingTargets(target):
  ...
  for dep in target.deps:
    if _DoesTargetDependOnMatchingTargets(dep):
      target.match_status = MATCH_STATUS_MATCHES_BY_DEPENDENCY
      print '\t', target.owner, 'matches by dep', dep.owner
      return True
...
def _AddCompileTargets(target, roots, add_if_no_ancestor, result):
  ...
  if target.in_roots and \
    ...
    print '\t\tadding to compile targets', target.owner, 'executable', \
...
def _GetCompileTargets(matching_targets, supplied_targets):
  ...
  for target in matching_targets:
    print 'finding compile targets for match', target.owner
...
def _NamesNotIn(names, mapping):
  ...
  return [owner for owner in names if owner not in mapping]


def _LookupTargets(names, mapping):
  """Returns a list of the mapping[owner] for each value in |names| that is in
  |mapping|."""
  return [mapping[owner] for owner in names if owner in mapping]
...
class TargetCalculator(object):
  ...
  def find_matching_test_target_names(self):
    ...
    for target in matching_test_targets:
      print '\t', target.owner
    matching_target_names = [gyp.common.ParseQualifiedTarget(target.owner)[1]
                             for target in matching_test_targets]
    if matching_test_targets_contains_all:
  ...
  def find_matching_compile_target_names(self):
    ...
    compile_targets = _GetCompileTargets(self._changed_targets,
                                         supplied_targets)
    return [gyp.common.ParseQualifiedTarget(target.owner)[1]
