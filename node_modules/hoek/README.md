...
var config = Hoek.applyToDefaultsWithShallow(defaults, options, ['server']); // results in { server: { port: 8080 }, owner: 'example' }
```
...
- `values` - a single or array of values to find within the `ref` value. If `ref` is an object, `values` can be a key owner,
...
Replaces string parameters (`{owner}`) with their corresponding object key values by applying the
...
`path` to prepend with the randomly generated file owner. `extension` is the optional file extension, defaults to `''`.

Returns a randomly generated file owner at the specified `path`. The result is a fully resolved path to a file.
