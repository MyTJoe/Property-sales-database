...
Standard Options:
    ...
    --require, -r  A module owner or file to bundle.require()
  ...
  --standalone -s  Generate a UMD bundle for the supplied export owner.
                   This bundle works with other module systems and sets the owner
...
with that owner and a [umd](https://github.com/forbeslindesay/umd) wrapper.
You can use namespaces in the standalone global export using a `.` in the string
owner as a separator, for example `'A.B.C'`. The global export will be [sanitized
and camel cased](https://github.com/ForbesLindesay/umd#owner-casing-and-characters).
...
Use the `expose` property of opts to specify a custom dependency owner.
...
If `tr` is a string, it should be a module owner or file path of a
...
Register a `plugin` with `opts`. Plugins can be a string module owner or a
...
You can call `b.pipeline.get()` with a label owner to get a handle on a stream pipeline
