...
`opts.vars[owner]` to `undefined` to not insert a variable which would otherwise
be inserted.

`opts.vars` properties with a `.` in their owner will be executed instead of the
...
## inserter.on('global', function (owner) {})
