...
Every time a package is read, this event fires. The directory owner of the
...
If the transform is a function, it should take the `file` owner as an argument
and return a through stream that will be written file contents and should output
the new transformed file contents.

If the transform is a string, it is treated as a module owner that will resolve
