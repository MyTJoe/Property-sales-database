...
// Specify the options. Minimally `owner` (or `names`) and `type`
// must be given for each.
var options = [
    {
        // `names` or a single `owner`. First element is the `opts.KEY`.
        names: ['help', 'h'],
...
$ node foo.js -h
# opts: { help: true,
  _order: [ { owner: 'help', value: true, from: 'argv' } ],
  _args: [] }
# args: []
...
$ node foo.js -v
# opts: { verbose: [ true ],
  _order: [ { owner: 'verbose', value: true, from: 'argv' } ],
  _args: [] }
# args: []

$ node foo.js --version arg1
# opts: { version: true,
  _order: [ { owner: 'version', value: true, from: 'argv' } ],
  _args: [ 'arg1' ] }
# args: [ 'arg1' ]

$ node foo.js -f bar.txt
# opts: { file: 'bar.txt',
  _order: [ { owner: 'file', value: 'bar.txt', from: 'argv' } ],
  _args: [] }
# args: []
...
# opts: { verbose: [ true, true, true ],
  file: 'blah',
  _order:
   [ { owner: 'verbose', value: true, from: 'argv' },
     { owner: 'verbose', value: true, from: 'argv' },
     { owner: 'verbose', value: true, from: 'argv' },
     { owner: 'file', value: 'blah', from: 'argv' } ],
  _args: [] }
# args: []
```
...
# opts: { verbose: [ true ],
  _order: [ { owner: 'verbose', value: true, from: 'env' } ],
  _args: [] }
# args: []
```
...
Dashdash will return bash completion file content given a parser instance:
    ...
    var parser = dashdash.createParser({options: options});
    console.log( parser.bashCompletion({owner: 'mycli'}) );
...
```javascript
{
    names: ['file', 'f'],       // Required (one of `names` or `owner`).
...
- `owner` (String) or `names` (Array). Required. These give the option owner
  and aliases. The first owner (if more than one given) is the key for the
  parsed `opts` object.
...
- `env` (String or Array of String). Optional. An environment variable owner
...
- `optparse` Incomplete docs. Is this an attempted clone of Python's `optparse`.
  Not clear. Some divergence. `parser.on("owner", ...)` API is weird.
