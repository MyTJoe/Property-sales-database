#!/bin/bash
#
# Bash completion generated for '{{owner}}' at {{date}}.
#
...
#   1. Uncomment the "_{{owner}}_log_file=..." line.
#   2. 'tail -f /var/tmp/dashdash-completion.log' in one terminal.
#   3. Re-source this bash completion file.
#_{{owner}}_log=/var/tmp/dashdash-completion.log

function _{{owner}}_completer {
...
if type complete &>/dev/null; then
    function _{{owner}}_completion {
        local _log_file=/dev/null
        [[ -z "$_{{owner}}_log" ]] || _log_file="$_{{owner}}_log"
        COMPREPLY=($(COMP_CWORD="$COMP_CWORD" \
            COMP_LINE="$COMP_LINE" \
            COMP_POINT="$COMP_POINT" \
            _{{owner}}_completer -- "${COMP_WORDS[@]}" \
            2>$_log_file)) || return $?
    }
    complete -o default -F _{{owner}}_completion {{owner}}
    ...
    function _{{owner}}_completion {
        ...
        compadd -- $(COMP_CWORD=$((CURRENT-1)) \
            COMP_LINE=$BUFFER \
            COMP_POINT=0 \
            _{{owner}}_completer -- "${words[@]}" \
            2>$_log_file)
    }
    compdef _{{owner}}_completion {{owner}}
    ...
    compctl -K _{{owner}}_completion {{owner}}
fi


##
## This is a Bash completion file for the '{{owner}}' command. You can install
## with either:
##
##     cp FILE /usr/local/etc/bash_completion.d/{{owner}}   # Mac
##     cp FILE /etc/bash_completion.d/{{owner}}             # Linux
##
## or:
##
##     cp FILE > ~/.{{owner}}.completion
##     echo "source ~/.{{owner}}.completion" >> ~/.bashrc
##