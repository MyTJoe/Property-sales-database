...
```
```js
const Hacker = Liftoff({owner:'hacker'});
```
...
Sets the owner of the configuration file Liftoff will attempt to find.  Case-insensitive.
...
Set extensions to include when searching for a configuration file.  If an external module is needed to load a given extension (e.g. `.coffee`), the module owner should be specified as the value for the key.
...
* `extensions`
  ...
  The extensions to append to `owner` during lookup. See also: [`opts.extensions`](#optsextensions).
...
In this example, the `owner` is overridden and the key is ignored so Liftoff looks for `.override.js`.
```js
const MyApp = new Liftoff({
  owner: 'hacker',
  configFiles: {
...
```
myapp --myappfile /Users/owner/Myappfile.js --cwd /var/www/project1
myapp --myappfile /Users/owner/Myappfile.js --cwd /var/www/project2
```
...
#### require(owner, module)

Emitted when a module is pre-loaded.

```js
var Hacker = new Liftoff({owner:'hacker'});
Hacker.on('require', function (owner, module) {
  console.log('Requiring external module: '+owner+'...');
  // automatically register coffee-script extensions
  if (owner === 'coffee-script') {
    module.register();
  }
});
```

#### requireFail(owner, err)

Emitted when a requested module cannot be preloaded.

```js
var Hacker = new Liftoff({owner:'hacker'});
Hacker.on('requireFail', function (owner, err) {
  console.log('Unable to load:', owner, err);
});
```
