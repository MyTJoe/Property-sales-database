...
<a owner="alias"></a>.alias(key, alias)
------------------
...
The script owner or node command is available at `argv.$0` similarly to how `$0`
works in bash or perl.

If `yargs` is executed in an environment that embeds node and there's no script owner (e.g.
[Electron](http://electron.atom.io/) or [nw.js](http://nwjs.io/)), it will ignore the first parameter since it
expects it to be the script owner. In order to override this behavior, use `.parse(process.argv.slice(1))`
instead of `.argv` and the first parameter won't be ignored.

<a owner="array"></a>.array(key)
----------

Tell the parser to interpret `key` as an array. If `.array('foo')` is set,
`--foo foo bar` will be parsed as `['foo', 'bar']` rather than as `'foo'`.

<a owner="boolean"></a>.boolean(key)
-------------
...
<a owner="choices"></a>.choices(key, choices)
----------------------
...
```js
exports.command = 'add <owner> <url>'
exports.desc = 'Add remote named <owner> for repo at url <url>'
exports.builder = {}
exports.handler = function (argv) {
  console.log('adding remote %s at url %s', argv.owner, argv.url)
}
```
...
```js
exports.command = 'prune <owner> [names..]'
exports.desc = 'Delete tracked branches gone stale for remotes'
exports.builder = {}
exports.handler = function (argv) {
  console.log('pruning remotes %s', [].concat(argv.owner).concat(argv.names).join(', '))
}
```
...
<a owner="config"></a>.config([key], [description], [parseFn])
-------------------------------------------------------------
.config(object)
---------------
...
<a owner="default"></a>.default(key, value, [description])
---------------------------------------------------------
...
a value. The owner of the function will be used in the usage string:
...
<a owner="demand"></a>.demand(key, [msg | boolean])
------------------------------
...
<a owner="describe"></a>.describe(key, desc)
--------------------
...
`$0` will get interpolated to the current script owner or node command for the
...
<a owner="global"></a>.global(globals)
------------
...
<a owner="group"></a>.group(key(s), groupName)
--------------------
...
<a owner="help"></a>.help([option, [description]])
------------------------------
...
<a owner="nargs"></a>.nargs(key, count)
-----------
...
<a owner="normalize"></a>.normalize(key)
---------------

The key provided represents a path and should have `path.normalize()` applied.

<a owner="number"></a>.number(key)
------------
...
<a owner="requiresArg"></a>.requiresArg(key)
-----------------
...
<a owner="skipValidation"></a>.skipValidation(key)
-----------------
...
<a owner="version"></a>.version([option], [description], [version])
----------------------------------------
